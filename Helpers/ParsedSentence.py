...
class ParsedSentence:
    ...
    def get_root_indexes(self) -> List[int]:
        indexes = []
        for np in self.NPs:
            indexes.append(np.root_index)
        return indexes

    def get_hypo_or_hyper_indexes(self, hypo_or_hyper: str) -> Set[int]:
        indexes = set()
        for NP in self.NPs:
    ...
    def get_lemma_by_index(self, ind: int) -> Optional[str]:
        for word in self.words:
            if word.index == ind:
                return word.lemma
        return None

    def get_sequence_representation(self, hypo_indexes=[], hyper_indexes=[]) -> str:
        if len(self.NPs) == 0:
            return "[]"
        s = "["
        np_i = 0
    ...
    def get_sequence_representation_for_SHyP(self) -> str:
        if len(self.NPs) == 0:
            return ""
        s = "["
        np_i = 0
        ...
        if len(hypo_indexes) > 0 and len(hyper_indexes) > 0:
            ...
            included_indexes = list(
                range(min(min(hypo_indexes), min(hyper_indexes)) - 5, max(max(hypo_indexes), max(hyper_indexes)) + 5))
        else:
            return ""
...
class ParsedWord:
    ...
    def __str__(self):
        return "(" + self.word + ", " + self.lemma + ", " + self.pos + ", " + str(
            self.index) + ", " + self.parent + ", " + str(
...
class NounPhrase:
    ...
    def __str__(self):
        return "(" + self.text + ", " + self.root + ", " + str(self.root_index) + ", " + str(self.start) + ", " + str(
            self.end) + ")"
